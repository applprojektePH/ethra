...
widgSelectBlockOptions.push("", "Block wählen");
...
widgEditor.prototype.writeDocument = function(documentContent)
{
	...
	var documentTemplate = '\
		<html>\
			...
			<body id="iframeBody" style="font-family: Raleway,sans-serif">\
				INSERT:CONTENT:END\
			</body>\
		</html>\
	';
...
function widgToolbar(theEditor)
{
	...
	for (var i = 0; i < widgToolbarItems.length; i++)
	{
		...
		{
			case "bold":
				this.addButton(this.theList.id + "ButtonBold", "widgButtonBold fas fa-bold fa-solid", "Bold", "bold");
				
				break;
				
			case "italic":
				this.addButton(this.theList.id + "ButtonItalic", "widgButtonItalic fas fa-solid fa-italic", "Italic", "italic");
				
				break;
				
			case "hyperlink":
				this.addButton(this.theList.id + "ButtonLink", "widgButtonLink fas fa-sharp fa-solid fa-link", "Hyperlink", "link");
				
				break;
				
			case "unorderedlist":
				this.addButton(this.theList.id + "ButtonUnordered", "widgButtonUnordered fas fa-solid fa-list", "Unordered List", "insertunorderedlist");
				
				break;
				
			case "orderedlist":
				this.addButton(this.theList.id + "ButtonOrdered", "widgButtonOrdered fas fa-solid fa-list-ol", "Ordered List", "insertorderedlist");
				
				break;

				
			case "htmlsource":
				this.addButton(this.theList.id + "ButtonHTML", "widgButtonHTML fas fa-file-code", "HTML Source", "html");
		...
		case "link":
			...
			else
			{
				if (theIframe.contentWindow.document.selection)
				{
					...
					if (theSelection == "")
					{
						alert("Markieren Sie den Text, den Sie in einen Link umwandeln möchten.");
				...
				var theURL = prompt("Link URL:", "http://");
